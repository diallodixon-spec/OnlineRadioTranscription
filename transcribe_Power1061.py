...
LOCK_FILE = "/root/speech2text/stream_onlineradio/transcribewave/tmp/transcription_script_power1061.lock"
...
def is_already_running():
        ...
        if os.path.exists(LOCK_FILE):
            try:
                ...
                if not pid_str:
                    ...
                    print("Found empty lock file for Power1061. Removing...")
                    log_message(f"Found empty lock file for Power1061. Removing...", log_file)
                    try:
                        os.remove(LOCK_FILE)
                    except:
                        pass
                ...
                try:
                    ...
                    print(f"Found invalid PID in lock file for Power1061: '{pid_str}'. Removing...")
                    log_message(f"Found invalid PID in lock file for Power1061: '{pid_str}'. Removing...", log_file)
                    try:
                        os.remove(LOCK_FILE)
                    except:
                        pass
                ...
                if psutil.pid_exists(pid):
                    if wait_time == 0:
                        print(f"Another instance is already running for Power1061 with PID {pid}.")
                        print(f"Waiting for lock for Power1061 to be released (checking every {LOCK_CHECK_INTERVAL}s)...")
                        log_message(f"Another instance is already running for Power1061 with PID {pid}. Waiting for lock for Power1061 to be released (checking every {LOCK_CHECK_INTERVAL}s)...", log_file)
                    else:
                        print(f"Still waiting(Power1061)... ({wait_time}s elapsed)")
                        log_message(f"Still waiting(Power1061)... ({wait_time}s elapsed)", log_file)
                    ...
                    print(f"Found stale lock file for Power1061 (PID {pid} not running). Removing...")
                    log_message(f"Found stale lock file for Power1061 (PID {pid} not running). Removing...", log_file)
                    try:
                        os.remove(LOCK_FILE)
                    except:
                        pass
                ...
                print(f"Error reading lock file for Power1061: {e}")
                if wait_time == 0:
                    print(f"Waiting and will retry(Power1061)... (checking every {LOCK_CHECK_INTERVAL}s)")
                ...
                # Unexpected error
                print(f"Unexpected error checking lock for Power1061: {e}")
                if wait_time == 0:
                    print(f"Waiting and will retry(Power1061)... (checking every {LOCK_CHECK_INTERVAL}s)")
    ...
    if wait_time >= MAX_LOCK_WAIT_TIME:
        print(f"Timeout: Lock not released for Power1061 after {MAX_LOCK_WAIT_TIME}s. Exiting.")
        return True
    ...
    try:
        ...
        print(f"Lock acquired for Power1061 (PID {os.getpid()})")
        return False
    except Exception as e:
        print(f"Error acquiring lock for Power1061: {e}")
        return True
...
# Configuration
HF_SPACE_URL = "https://dmajor007-audiotranscription2-hfrepo.hf.space"
