    ...
    try:
        resp = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "user",
                    "content": f"The provided texts are transcripts of a radio program. the transcriptions were done in 1 minute segments. provide bullet point summary of each segment:\n\n{content}"
                }
            ]
        )
        ...
        resp2 = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "user",
                    "content": f"The provided texts are transcripts of a radio program. the transcriptions were done in 1 minute segments. treat all segments as one body of text, and proivde the topic or topics based on the transcription. include the time range for the topics. similar topics discussed in different time ranges can be merged into the same topic, with the multiple time ranges included. where appropriate, discussions on jamaica should be separate from other countries:\n\n{content}"
                }
            ]
        )
        #return message.content[0].text
        # Extract the model's message content
        content = None
        if resp2.choices and hasattr(resp2.choices[0], "message"):
