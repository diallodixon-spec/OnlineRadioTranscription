...
LOCK_FILE = "/root/speech2text/stream_onlineradio/transcribewave/tmp/transcription_script_power1062.lock"
...
def is_already_running():
        ...
        if os.path.exists(LOCK_FILE):
            try:
                ...
                if not pid_str:
                    ...
                    print("Found empty lock file for Power1062. Removing...")
                    try:
                        os.remove(LOCK_FILE)
                    except:
                        pass
                ...
                try:
                    ...
                    print(f"Found invalid PID in lock file for Power1062: '{pid_str}'. Removing...")
                    try:
                        os.remove(LOCK_FILE)
                    except:
                        pass
                ...
                if psutil.pid_exists(pid):
                    if wait_time == 0:
                        print(f"Another instance is already running for Power1062 with PID {pid}.")
                        print(f"Waiting for lock for Power1062 to be released (checking every {LOCK_CHECK_INTERVAL}s)...")
                    else:
                        print(f"Still waiting(Power1062)... ({wait_time}s elapsed)")
                    ...
                    print(f"Found stale lock file for Power1062 (PID {pid} not running). Removing...")
                    try:
                        os.remove(LOCK_FILE)
                    except:
                        pass
                ...
                print(f"Error reading lock file for Power1062: {e}")
                if wait_time == 0:
                    print(f"Waiting and will retry(Power1062)... (checking every {LOCK_CHECK_INTERVAL}s)")
                ...
                # Unexpected error
                print(f"Unexpected error checking lock for Power1062: {e}")
                if wait_time == 0:
                    print(f"Waiting and will retry(Power1062)... (checking every {LOCK_CHECK_INTERVAL}s)")
    ...
    if wait_time >= MAX_LOCK_WAIT_TIME:
        print(f"Timeout: Lock not released for Power1062 after {MAX_LOCK_WAIT_TIME}s. Exiting.")
        return True
    ...
    try:
        ...
        print(f"Lock acquired for Power1062 (PID {os.getpid()})")
        return False
    except Exception as e:
        print(f"Error acquiring lock for Power1062: {e}")
        return True
...
# Configuration
HF_SPACE_URL = "https://dmajor007-audiotranscription3-hfrepo.hf.space"
