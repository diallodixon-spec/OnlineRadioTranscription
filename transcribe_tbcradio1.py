...
LOCK_FILE = "/root/speech2text/stream_onlineradio/transcribewave/tmp/transcription_script_tbcradio1.lock"
...
def is_already_running():
        ...
        if os.path.exists(LOCK_FILE):
            try:
                ...
                if not pid_str:
                    ...
                    print("Found empty lock file for tbcradio1. Removing...")
                    try:
                        os.remove(LOCK_FILE)
                    except:
                        pass
                ...
                try:
                    ...
                    print(f"Found invalid PID in lock file for tbcradio1: '{pid_str}'. Removing...")
                    try:
                        os.remove(LOCK_FILE)
                    except:
                        pass
                ...
                if psutil.pid_exists(pid):
                    if wait_time == 0:
                        print(f"Another instance is already running for tbcradio1 with PID {pid}.")
                        print(f"Waiting for lock for tbcradio1 to be released (checking every {LOCK_CHECK_INTERVAL}s)...")
                    else:
                        print(f"Still waiting(tbcradio1)... ({wait_time}s elapsed)")
                    ...
                    print(f"Found stale lock file for tbcradio1 (PID {pid} not running). Removing...")
                    try:
                        os.remove(LOCK_FILE)
                    except:
                        pass
                ...
                print(f"Error reading lock file for tbcradio1: {e}")
                if wait_time == 0:
                    print(f"Waiting and will retry(tbcradio1)... (checking every {LOCK_CHECK_INTERVAL}s)")
                ...
                # Unexpected error
                print(f"Unexpected error checking lock for tbcradio1: {e}")
                if wait_time == 0:
                    print(f"Waiting and will retry(tbcradio1)... (checking every {LOCK_CHECK_INTERVAL}s)")
    ...
    if wait_time >= MAX_LOCK_WAIT_TIME:
        print(f"Timeout: Lock not released for tbcradio1 after {MAX_LOCK_WAIT_TIME}s. Exiting.")
        return True
    ...
    try:
        ...
        print(f"Lock acquired for tbcradio1 (PID {os.getpid()})")
        return False
    except Exception as e:
        print(f"Error acquiring lock for tbcradio1: {e}")
        return True
...
# Configuration
HF_SPACE_URL = "https://dillydbak-audiotranscription7-hfrepo.hf.space"

# Directories
TBCRADIO_RECORDINGS_DIR = "/root/speech2text/stream_onlineradio/transcribewave/recordings_tbcradio"
TBCRADIO_TRANSCRIPTS_DIR = "/root/speech2text/stream_onlineradio/transcribewave/transcripts_tbcradio"
TBCRADIO_COMPLETED_DIR = "/root/speech2text/stream_onlineradio/transcribewave/recordings_processed_tbcradio"
LOG_DIR = "/root/speech2text/stream_onlineradio/transcribewave/logs"
...
def transcribe_single_file(client, wav_file, transcript_path, completed_dir, log_file):
    ...
    try:
        ...
        with open(transcript_path, 'w', encoding='utf-8') as f:
            ...
            f.write(f"Source: {original_wav_name}\n")
...
def process_directory(station_name, recordings_dir, transcripts_dir, completed_dir, client, log_file):
    ...
    log_message(f"📻 Processing {station_name} via tbcradio1", log_file)
    ...
    log_message("-" * 60, log_file)
    log_message(f"📊 {station_name} Results(tbcradio1): {successful} successful, {failed} failed", log_file
    log_message("=" * 60, log_file)
...
def main():
    ...
    # Process recordings
    if os.path.exists(TBCRADIO_RECORDINGS_DIR):
        successful, failed = process_directory(
            "TBCRadio", 
            TBCRADIO_RECORDINGS_DIR, 
            TBCRADIO_TRANSCRIPTS_DIR,
            TBCRADIO_COMPLETED_DIR,
            client, 
            log_file
        )
        ...
        log_message(f"⚠️  Directory not found: {TBCRADIO_RECORDINGS_DIR}", log_file)
