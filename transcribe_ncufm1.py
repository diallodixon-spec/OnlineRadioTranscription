...
LOCK_FILE = "/root/speech2text/stream_onlineradio/transcribewave/tmp/transcription_script_ncufm1.lock"
...
def is_already_running():
        ...
        if os.path.exists(LOCK_FILE):
            try:
                ...
                if not pid_str:
                    ...
                    print("Found empty lock file for ncufm1. Removing...")
                    try:
                        os.remove(LOCK_FILE)
                    except:
                        pass
                ...
                try:
                    ...
                    print(f"Found invalid PID in lock file for ncufm1: '{pid_str}'. Removing...")
                    try:
                        os.remove(LOCK_FILE)
                    except:
                        pass
                ...
                if psutil.pid_exists(pid):
                    if wait_time == 0:
                        print(f"Another instance is already running for ncufm1 with PID {pid}.")
                        print(f"Waiting for lock for ncufm1 to be released (checking every {LOCK_CHECK_INTERVAL}s)...")
                    else:
                        print(f"Still waiting(ncufm1)... ({wait_time}s elapsed)")
                    ...
                    print(f"Found stale lock file for ncufm1 (PID {pid} not running). Removing...")
                    try:
                        os.remove(LOCK_FILE)
                    except:
                        pass
                ...
                print(f"Error reading lock file for ncufm1: {e}")
                if wait_time == 0:
                    print(f"Waiting and will retry(ncufm1)... (checking every {LOCK_CHECK_INTERVAL}s)")
                ...
                # Unexpected error
                print(f"Unexpected error checking lock for ncufm1: {e}")
                if wait_time == 0:
                    print(f"Waiting and will retry(ncufm1)... (checking every {LOCK_CHECK_INTERVAL}s)")
    ...
    if wait_time >= MAX_LOCK_WAIT_TIME:
        print(f"Timeout: Lock not released for ncufm1 after {MAX_LOCK_WAIT_TIME}s. Exiting.")
        return True
    ...
    try:
        ...
        print(f"Lock acquired for ncufm1 (PID {os.getpid()})")
        return False
    except Exception as e:
        print(f"Error acquiring lock for ncufm1: {e}")
        return True
...
# Configuration
HF_SPACE_URL = "https://dillydbak-audiotranscription1-hfrepo.hf.space"
...
def process_directory(station_name, recordings_dir, transcripts_dir, completed_dir, client, log_file):
    ...
    log_message(f"📻 Processing {station_name} via ncufm1", log_file)
    ...
    log_message("-" * 60, log_file)
    log_message(f"📊 {station_name} Results(ncufm1): {successful} successful, {failed} failed", log_file)
    log_message("=" * 60, log_file)
